...
// Body: { mlEventId, videoId, seatId, objectDetected, objectConfidence,
//         timestamps, bboxOverlay, activities, confidenceScore, personIds,
//         duration }
// (shape produced by event_adapter.adapt_bridge_event_to_node_payload --
// the single write path for ML events; ml-service/pipeline_runner.py no
// longer writes to the events collection directly, see its own comments)
router.post('/events', async (req, res, next) => {
  try {
    const {
      mlEventId, videoId, seatId, objectDetected, objectConfidence,
      timestamps, bboxOverlay, activities, confidenceScore, personIds, duration,
    } = req.body;
